 ...
 * it to your needs.
 */
public interface AgentInstance<T> {
    /**
     * This message is sent to request creation of an agent instance. Implementations may, at their discretion choose to not spin up an agent instance.
     * <p>
    ...
    /**
     * This message is sent from the {@link ServerPingRequestExecutor} to terminate instances that did not register with the
     * server after a timeout. The timeout may be configurable and set via the {@link PluginSettings} instance that is passed in.
     *
     * @param settings the plugin settings object
    ...
    /**
     * This message is sent from the {@link ServerPingRequestExecutor} to filter out any new agents, that have registered
     * before the timeout period. The timeout may be configurable and set via the {@link PluginSettings} instance that is passed in.
     *
     * @param settings the plugin settings object
     * @param agents   the list of all the agents
    ...
    /**
     * This message is sent after plugin initialization time so that the plugin may connect to the cloud provider and fetch a list of all instances that have
     * been spun up by this plugin (before the server was shut down). This call should be should ideally remember if the agent instances are refreshed, and do
     * nothing if instances were previously refreshed.
     *
    ...
    /**
     * This Returns an agent instance with the specified <code>id</code> or <code>null</code>, if the agent is not found.
     *
     * @param agentId the elastic agent id
     */
